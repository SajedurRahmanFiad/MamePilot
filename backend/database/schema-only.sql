...
-- Statements tagged "-- @keep-data-backfill" are retained: routine derived-data

-- backfills that only fill NULL columns and are safe to re-run on every deployment.
...
CREATE TABLE IF NOT EXISTS company_settings (
  ...
  height_unit VARCHAR(16) NOT NULL DEFAULT 'cm',
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CALL sp_drop_fk('batches', 'fk_batches_category');

CALL sp_add_fk('batches', 'fk_batches_category', '(`category_id`)', 'categories', '(`id`)', 'ON DELETE SET NULL');
...
-- Migration: 2026-08-15_order_status_timestamp_columns.sql
...
CALL sp_create_idx('orders', 'idx_orders_processed_at', '`processed_at`');
CALL sp_create_idx('orders', 'idx_orders_courier_assigned_at', '`courier_assigned_at`');
CALL sp_create_idx('orders', 'idx_orders_picked_at', '`picked_at`');
CALL sp_create_idx('orders', 'idx_orders_completed_at', '`completed_at`');
CALL sp_create_idx('orders', 'idx_orders_returned_at', '`returned_at`');
CALL sp_create_idx('orders', 'idx_orders_cancelled_at', '`cancelled_at`');
CALL sp_create_idx('orders', 'idx_orders_partial_delivered_at', '`partial_delivered_at`');
CALL sp_create_idx('orders', 'idx_orders_exchange_processing_at', '`exchange_processing_at`');
CALL sp_create_idx('orders', 'idx_orders_exchange_picked_at', '`exchange_picked_at`');
CALL sp_create_idx('orders', 'idx_orders_exchange_delivered_at', '`exchange_delivered_at`');
...
-- Migration: 2026-08-26_delivery_action_required.sql
CALL sp_add_col('orders', 'delivery_action_required', 'TINYINT(1) NOT NULL DEFAULT 0');
...
-- Migration: 2026-09-23_sms_service.sql

...
final class AgentQueueScheduler
{
    private const MARKER_PREFIX = '# mamepilot-agent-queue';
    ...
    public function ensureInstalled(): array
    {
        if (DIRECTORY_SEPARATOR === '\\') {
            return ['status' => 'skipped', 'message' => 'Agent queue schedule installation is not used on Windows.'];
        }
        if (!$this->boolConfig('AGENT_QUEUE_MANAGE_CRON', true)) {
            return ['status' => 'skipped', 'message' => 'Agent queue schedule management is disabled.'];
        }
        if (!function_exists('proc_open')) {
            return ['status' => 'unavailable', 'message' => 'The hosting account does not allow agent queue schedule management.'];
        }
        ...
        if (self::normalizeCrontab($existing) === self::normalizeCrontab($merged)) {
            return ['status' => 'present', 'message' => 'The agent queue schedule is already installed.'];
        }

        $installed = $this->run(['crontab', '-'], $merged);
        if ($installed['exitCode'] !== 0) {
            return ['status' => 'unavailable', 'message' => 'The agent queue schedule could not be installed automatically.'];
        }

        return ['status' => 'installed', 'message' => 'The agent queue schedule was installed.'];
    }
    ...
    {
        $script = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'bin' . DIRECTORY_SEPARATOR . 'process_agent_queue.php';
        $marker = self::MARKER_PREFIX . ':' . substr(hash('sha256', self::normalizePath($script)), 0, 12);
        $logPath = trim((string) ($this->config->get('AGENT_QUEUE_WORKER_LOG', '') ?? ''));
        if ($logPath === '') {
            $logPath = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'mamepilot-agent.log';
        }

        return sprintf(
            '* * * * * %s %s --max-runs 10 >> %s 2>&1 %s',
    ...
    {
        if (preg_match('/# mamepilot-agent-queue:[a-f0-9]{12}/i', $entry, $matches) === 1) {
    ...
    {
        if (preg_match('/([\'\"])([^\'\"]*process_agent_queue\.php)\1/', $entry, $matches) === 1) {
            return (string) $matches[2];
        }
        if (preg_match('/([^\s\'\"]*process_agent_queue\.php)/', $entry, $matches) === 1) {

...
const Dashboard: React.FC = () => {
  ...
  const { hasCapability, hasSubCapability, settings: capabilitySettings } = useCapabilities(Boolean(user));
              ...
              {adminSnapshot.actionRequiredOrders.map((order) => (
                <div key={order.id} className="flex items-center justify-between border-b border-gray-50 pb-3 last:border-b-0">
                  <div className="min-w-0">
                    ...
                    <span className="text-xs font-medium text-gray-500">{order.customerName || `Unknown ${terminology.customerLower}`}</span>
                  </div>
  ...
  const getWidgetWidthStyle = (setting: { key: string; widthPercent?: number }): React.CSSProperties => {
    ...
    if (!isDesktop) return { flex: '0 0 100%', minWidth: 0 };
    const gapAdjustment = percent >= 100 ? '0rem' : `${percent * 0.015}rem`;
    return { flex: `0 0 calc(${percent}% - ${gapAdjustment})`, minWidth: 0 };
  };

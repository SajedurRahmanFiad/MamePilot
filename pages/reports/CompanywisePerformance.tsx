...
const MetricRow: React.FC<{ label: string; value: string | number; isBold?: boolean; isTotal?: boolean; isCurrency?: boolean }> = ({ label, value, isBold, isTotal, isCurrency = true }) => (
  <div className={`flex justify-between py-2.5 ${isBold ? 'font-bold text-gray-900' : 'text-gray-600'} ${isTotal ? 'border-t-2 border-gray-100 pt-4 mt-2' : ''}`}>
    <span className="text-sm">{label}</span>
    <span className="text-sm font-black">{isCurrency ? formatCurrency(value as number) : value}</span>
  </div>
);

const CompanywisePerformance: React.FC = () => {
  ...
  const { data: reportData, isPending: isReportLoading } = useCompanywisePerformanceReport(
    reportFilterRange,
  ...
  if (isCompanyLoading || isReportLoading) {
    return <ReportPageSkeleton cards={4} showChart={false} showFilters tableColumns={2} tableRows={6} />;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-bold text-gray-900">Companywise Performance</h2>
        </div>
        <button
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="p-8 space-y-2">
          <div className="flex gap-4 pb-6 mb-2 border-b border-gray-100 flex-wrap">
            <div className="flex-1 min-w-[120px] text-center">
              <p className="text-2xl font-black text-gray-900">{reportData?.deliveredOrders ?? 0}</p>
              <p className="text-xs font-bold text-gray-500 mt-1">Delivered Orders</p>
            </div>
            <div className="flex-1 min-w-[120px] text-center border-l border-gray-100 pl-4">
              <p className="text-2xl font-black text-gray-900">{reportData?.exchangeDeliveredOrders ?? 0}</p>
              <p className="text-xs font-bold text-gray-500 mt-1">Exchange Delivered</p>
            </div>
            <div className="flex-1 min-w-[120px] text-center border-l border-gray-100 pl-4">
          ...
          <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Income</h4>
          <MetricRow label="Income from Delivered & Exchange Delivered Orders" value={reportData?.income ?? 0} />
          {(reportData?.nonDeliveredIncome ?? 0) > 0 && (
            <MetricRow label="Income from Other Orders (Pending, Processing, etc.)" value={reportData!.nonDeliveredIncome} isBold />
          )}

          <div className="pt-6">
            <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Expenses</h4>
            <MetricRow label="Shipping Cost (Courier Charges)" value={reportData?.shippingCost ?? 0} />
            <MetricRow label="Cost of Goods Sold (COGS)" value={reportData?.cogs ?? 0} />
          </div>

          <div className="pt-12">
            <div className={`p-6 rounded-lg flex justify-between items-center ${(reportData?.profit ?? 0) >= 0 ? theme.colors.primary[600] : 'bg-red-600'} text-white shadow-xl`}>
              <span className="text-lg font-black uppercase tracking-widest">Profit / Loss</span>
              <span className="text-lg font-black">{formatCurrency(reportData?.profit ?? 0)}</span>
            </div>
          </div>
        </div>
...
export default CompanywisePerformance;

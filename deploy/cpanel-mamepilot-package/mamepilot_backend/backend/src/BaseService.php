...
abstract class BaseService
{
    ...
    protected const DEFAULT_ROLE_PERMISSIONS = [
        'Employee' => [
            ...
            'leads.edit' => true,
            'products.view' => true,
            'fraudChecker.check' => true,
            'wallet.view' => true,
            'reports.view' => true,
            'reports.viewExpense' => true,
            'reports.viewIncome' => true,
            'reports.viewProfitLoss' => true,
            'reports.viewCustomerSales' => true,
            'reports.viewProductQuantity' => true,
            'reports.viewUserActivity' => true,
        ],
    ];
    ...
    protected function mapCustomer(array $row): array
    {
        return [
            ...
            'dateOfBirth' => $this->toIso($row['date_of_birth'] ?? null),
            ...
            'guardianName' => $this->nullableString($row['guardian_name'] ?? null),
            'emergencyContact' => $this->nullableString($row['emergency_contact'] ?? null),
            'additionalNotes' => $this->nullableString($row['additional_notes'] ?? null),
            ...
            'fraudCheckResult' => $this->jsonDecodeAssoc($row['fraud_check_result'] ?? null) ?: null,
            'fraudCheckPercentage' => isset($row['fraud_check_percentage']) ? (float) $row['fraud_check_percentage'] : null,
            'fraudCheckPhone' => $this->nullableString($row['fraud_check_phone'] ?? null),
            'fraudCheckedAt' => $this->toIso($row['fraud_checked_at'] ?? null),
    ...
    protected function mapProduct(array $row): array
    {
        return [
            ...
            'batchLotNumber' => $this->nullableString($row['batch_lot_number'] ?? null),
            'expiryDate' => $this->toIso($row['expiry_date'] ?? null),
            'recommendedDoseSequence' => $this->nullableString($row['recommended_dose_sequence'] ?? null),
    ...
    protected function mapOrder(array $row): array
    {
        ...
        return [
            ...
            'vatRate' => (float) ($row['vat_rate'] ?? $row['vatRate'] ?? 0),
            ...
            'deliveryActionRequired' => (bool) ($row['delivery_action_required'] ?? $row['deliveryActionRequired'] ?? false),
            ...
            'surveyNextRetryAt' => $this->toIso($row['survey_next_retry_at'] ?? $row['surveyNextRetryAt'] ?? null),
            'surveyTriggeredAt' => $this->toIso($row['survey_triggered_at'] ?? $row['surveyTriggeredAt'] ?? null),
            'surveyLastRetryReason' => $this->nullableString($row['survey_last_retry_reason'] ?? $row['surveyLastRetryReason'] ?? null),
            'surveyLastRetryAt' => $this->toIso($row['survey_last_retry_at'] ?? $row['surveyLastRetryAt'] ?? null),

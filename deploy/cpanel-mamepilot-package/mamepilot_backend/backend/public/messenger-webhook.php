...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'GET') {
    $challenge = $messenger->webhookVerification(
...
try {
    $result = $messenger->handleWebhook((string) $rawBody, $_SERVER['HTTP_X_HUB_SIGNATURE_256'] ?? null);
    http_response_code(200);
    header('Content-Type: application/json; charset=utf-8');
    echo json_encode($result, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
} catch (Throwable $exception) {
    http_response_code($exception->getMessage() === 'Invalid Messenger webhook signature.' ? 403 : 400);
    header('Content-Type: application/json; charset=utf-8');
